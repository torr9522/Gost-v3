...
| 场景 | 自动放行 UFW | 自动 reload |
| --- | --- | --- |
| 新增普通代理端口 | 放行监听端口 `tcp` | 重建配置时重启 gost |
| 新增 HTTPS 代理 | 放行监听端口 `tcp`，额外放行 `80/tcp` | 证书安装后 `reloadcmd` 自动重启 gost |
| 删除最后一条 HTTPS 规则 | 业务端口回收；若无 HTTPS 规则则 `80/tcp` 也会从脚本管理集移除 | 重建配置时重启 gost |
